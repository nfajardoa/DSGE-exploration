...
for (s in 1:3){
  ...
  for (i in first.sector[s+1]:last.sector[s+1]) {
    assign(paste0("out_",i) ,filter(datos, industry==names[i]) %>% ggplot(aes(y=value, x=year)) +  geom_line(group = 1, size=1) + geom_point() + labs(title="Participación del trabajo sobre el valor agregado (Sin ingreso mixto)", subtitle=paste(names[i]), y="Porcentaje", x="Año"))
  ...
  plot <- do.call("arrangeGrob", group)
  ggsave(plot, file=paste0("consolidado_participacion_sim_",s,".png"), width = 25, height = 15, units = "in", dpi = 300, limitsize = FALSE)
...
for (s in 1:3) {
  ...
  ggsave(paste0("sector_",s,"_Participacion_SIM.png"), width = 15, height = 5, units = "in", dpi = 300, limitsize = FALSE)
}
