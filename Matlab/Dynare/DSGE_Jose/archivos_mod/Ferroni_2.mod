var y c k h e_a e_b gy_obs gh_obs;
varexo v_a v_b;
...
gy_obs=(y/y(-1))*exp(e_a+e_b);

gh_obs=(h/h(-1))*exp(e_b);
...
estimation(datafile=variables_simuladas, bayesian_irf, mode_check, mh_conf_sig=0.95 ,mh_replic=20000,selected_variables_only, mode_compute=6) gy_obs gh_obs;